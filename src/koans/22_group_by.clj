...
(defn get-odds-and-evens [coll]
    ; spot a map in a let & you're doing associative destructuring:
    ; note true & false treated here as ordinary symbols(?)
    (let [{odds true evens false} (group-by odd? coll)] ; gives { true [1 3 5] false [2 4] }
    [odds evens]))
...
(meditations
  ...
  (= (get-odds-and-evens [1 2 3 4 5])
     ((juxt filter remove) odd? [1 2 3 4 5]) ; makes one coll filtered with odd?,
     [[1 3 5] [2 4]])                        ; another, with remove, then inversion
                                             ; as pictured.
  ...
  (= {"Bob" [{:name "Bob" :id 1}]
      ...
      nil [{:last-name "Smith" :id 1}]}   ; exactly as would expect. nice.
